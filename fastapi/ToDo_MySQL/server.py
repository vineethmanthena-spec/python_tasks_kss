...
def create_todo(todo: Todo, db: Session = Depends(get_db)):
    ...
    return {"message": "Created", "data": Todo.model_validate(new_todo)}
...
def get_all(db: Session = Depends(get_db)):
    ...
    return {"count": len(todos), "data": [Todo.model_validate(t) for t in todos]}
...
def update(todo_id: int, updated: Todo, db: Session = Depends(get_db)):
    ...
    return {"message": "Updated", "data": Todo.model_validate(todo)}
